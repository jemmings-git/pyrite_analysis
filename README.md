 ...
 * `SGP_Fe_ferruginous_fractions.csv` - the fraction of ferruginous vs. euxinic conditions on an age (1 Ma bin) and long-lat basis, suited for coarse spatial and temporal interpolation, derived from [SGP Fe-speciation data](http://sgp-search.io/). These data were interpolated using reconstructed palaeogeographic coordinates. Presented on **Figure 6**
 * `SGP_Fe_ferruginous_fractions_strat.csv` - the fraction of ferruginous vs. euxinic conditions on an age (1 Ma bin), long-lat basis *and* stratigraphic name, derived from [SGP Fe-speciation data](http://sgp-search.io/). These data were matched to the pyrite TE record (see below). 
