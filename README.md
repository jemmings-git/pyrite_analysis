 ...
 * Intermediary xDD results (*data_part2_comp.csv* - composite xDD pyrite outputs as in Fig 1A-B, *data_part1_comp.csv* - strat-only xDD pyrite outputs as in Fig S3A-B, *wilkin_framboids_comp.csv* - wilkin-pyrite composite results as in Fig. 1C, *wilkin_framboids_strat.csv* - wilkin-pyrite strat-only results as in Fig. 3C) can be generated using the R code in this directory, or the user may wish to skip the processing steps & import the files directly (copies are stored in this directory) ![#c5f015](https://via.placeholder.com/15/c5f015/000000?text=+)
